...
function verif_values(){ //this function will verify required values and syntax.
    ...
    $date_ajout = check_values($_POST['pro_d_ajout']);
    ...
    $alpha6 ="/^(?:19|20)[0-9]{2}-(?:(?:0[1-9]|1[0-2])-(?:0[1-9]|1[0-9]|2[0-9])|(?:(?!02)(?:0[1-9]|1[0-2])-(?:30))|(?:(?:0[13578]|1[02])-31))\s[0-1][0-9]:[0-5][0-9]:[0-5][0-9]$/"; //For pro-d-ajout &pro_d_modif date

    if(isset($_POST['submit']))
    {
        // TODO: Terminer la manip vérification erreur tableau
        $tab_verif =[];
        ...
        if (empty($date_ajout))
        {
            ...
            echo "Date d'ajout du produit = $date_ajout <br>";
        }

        // Vérification du bloquage ou non du produit TODO: mettre un ou "!= 0"
        if (empty($bloque))
        {
    ...
    try
    {
        ...
        $db = connexionBase();
        $requete = $db->prepare("INSERT INTO produits (`pro_id`, `pro_cat_id`, `pro_ref`, `pro_libelle`, `pro_description`, `pro_prix`, `pro_stock`, `pro_couleur`, `pro_photo`, `pro_d_ajout`, `pro_bloque`) 
        VALUES (:pro_id, :pro_cat_id, :pro_ref, :pro_libelle, :pro_description, :pro_prix, :pro_stock, :pro_couleur, :pro_photo, :pro_d_ajout, :pro_bloque)");
